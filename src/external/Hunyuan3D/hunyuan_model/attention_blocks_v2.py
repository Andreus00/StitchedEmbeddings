...
from hunyuan_model.attention_processor import CrossAttentionProcessor
...
class PointCrossAttentionEncoder(nn.Module):
    ...
    def __init__(
        ...
        # if pc_boundary_size == 0:
        #     print(
        #         'PointCrossAttentionEncoder INFO: pc_sharpedge_size is zero')
        # else:
        #     print(
        #         f'PointCrossAttentionEncoder INFO: pc_boundary_size is given, using pc_size={pc_size}, pc_boundary_size={pc_boundary_size}')

        self.pc_size = pc_size
        self.pc_sharpedge_size = pc_sharpedge_size
        self.pc_boundary_size = pc_boundary_size
    ...
    def sample_points_and_latents(self, pc: torch.FloatTensor, feats: Optional[torch.FloatTensor] = None):
        ...
        random_pc, sharpedge_pc, boundary_pc = torch.split(pc, [self.pc_size, self.pc_sharpedge_size, self.pc_boundary_size], dim=1)
        ...
        assert boundary_pc.shape[
                   1] <= self.pc_boundary_size, "Boundary surface points size must be less than or equal to pc_boundary_size"
        ...
        # Randomly select sharpedge surface points and sharpedge query points
        input_boundary_pc_size = int(num_boundary_query * self.downsample_ratio)
        if input_boundary_pc_size == 0:
            input_boundary_pc = torch.zeros(B, 0, D, dtype=input_random_pc.dtype).to(pc.device)
            query_boundary_pc = torch.zeros(B, 0, D, dtype=query_random_pc.dtype).to(pc.device)
        else:
            boundary_query_ratio = num_boundary_query / input_boundary_pc_size
            idx_boundary_pc = torch.randperm(boundary_pc.shape[1], device=boundary_pc.device)[
                               :input_boundary_pc_size]
            input_boundary_pc = boundary_pc[:, idx_boundary_pc, :]
            flatten_input_boundary_surface_points = input_boundary_pc.view(B * input_boundary_pc_size, D)
            N_down = int(flatten_input_boundary_surface_points.shape[0] / B)
            batch_down = torch.arange(B).to(pc.device)
            batch_down = torch.repeat_interleave(batch_down, N_down)
            idx_query_boundary = fps(flatten_input_boundary_surface_points, batch_down, ratio=boundary_query_ratio)
        ...
        if self.point_feats != 0:
            ...
            random_surface_feats, sharpedge_surface_feats, boundary_surface_feats = torch.split(feats, [self.pc_size, self.pc_sharpedge_size, self.pc_boundary_size],
                                                                        dim=1)
            ...
            if input_boundary_pc_size == 0:
                input_boundary_surface_feats = torch.zeros(B, 0, self.point_feats,
                                                            dtype=input_random_surface_feats.dtype).to(pc.device)
                query_boundary_feats = torch.zeros(B, 0, self.point_feats, dtype=query_random_feats.dtype).to(
                    pc.device)
            else:
                input_boundary_surface_feats = boundary_surface_feats[:, idx_boundary_pc, :]
                flatten_input_boundary_surface_feats = input_boundary_surface_feats.view(B * input_boundary_pc_size,
                                                                                           -1)
                query_boundary_feats = flatten_input_boundary_surface_feats[idx_query_boundary].view(B, -1,
                                                                                                        flatten_input_boundary_surface_feats.shape[
                                                                                                            -1])

            query_feats = torch.cat([query_random_feats, query_sharpedge_feats, query_boundary_feats], dim=1)
            input_feats = torch.cat([input_random_surface_feats, input_sharpedge_surface_feats, input_boundary_surface_feats], dim=1)
        ...
        if input_boundary_pc_size == 0:
            query_boundary_pc = torch.zeros(B, 1, D).to(pc.device)
            input_boundary_pc = torch.zeros(B, 1, D).to(pc.device)
