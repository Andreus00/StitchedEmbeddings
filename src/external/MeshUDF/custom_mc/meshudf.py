...
from src.external.MeshUDF.custom_mc._marching_cubes_lewiner import udf_mc_lewiner
...
class GridFiller:
    ...
    def __init__(
        ...
        overall_index = torch.arange(0, self.N_max**3, 1, out=torch.LongTensor(device='cpu'), device='cpu')
        samples = torch.zeros(self.N_max**3, 7, device='cpu')
        ...
        """
        mask = torch.zeros(self.N_max**3).bool().to(self.device)
        ...
        for i, N in enumerate(self.N_levels):
            ...
            self.masks_coarse[i] = mask_coarse.clone().to(self.device)
            ...
            self.idxs_coarse_neighbors_blocks[
                i
            ] = idxs_coarse_neighbors_blocks.clone().to(self.device)
            ...
            if i > 0:
                ...
                self.masks_coarse_no_recompute[
                    i
                ] = mask_coarse_no_recompute.clone().to(self.device)
    ...
    def fill_grid(
        ...
        with torch.no_grad():
            ...
            for level, N in enumerate(self.N_levels):
                ...
                """ Query the network """
                xyz = samples[mask_coarse_no_recompute, 0:3].to(self.device)
                # Query and fill grid.
...
def get_mesh_from_udf(
    ...
    vertices, faces, _, _ = udf_mc_lewiner(
        udf, 
        gradients, 
        spacing=[spacing] * 3,
        th_alpha=th_alpha,
    ...
    # return torch.asarray(mesh.vertices).contiguous(), torch.asarray(mesh.faces).contiguous()
    ...
    if smooth_borders:
        ...
        border_vertices = np.array(list(neighbours.keys())).astype(np.int32)
    ...
    return final_verts.detach().cpu(), final_faces.detach().cpu()
